...
def lambda_handler(event, context):
    ...
    try:
        ...
        #print('before split')
        #print("authorization: " + authorization_header['authorization'])
        ...
        hash_as_bytes = username_password_hash.encode("utf-8")
        #print('hash_as_bytes')
        #print(hash_as_bytes)
        decoded_str = base64.standard_b64decode(hash_as_bytes).decode("utf-8")
        username = decoded_str.split(':')[0]
        password = decoded_str.split(':')[1]
        print("username: " + username)
        #digest = gethmacdigest(username)
        # Call Cognito
        cognito_auth_response = client.initiate_auth(
        ...
        if cognito_auth_response['ResponseMetadata']['HTTPStatusCode'] == 200:
            print("Successful authentication against Cognito User Pool!")
            policy.allowMethod(event['requestContext']['httpMethod'], event['path'])
        else:
            print("Failed to authenticate against Cognito User Pool")
